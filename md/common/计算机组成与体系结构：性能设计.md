...
1. 多处理器组织
   1. 并行处理器系统的类型
      1. 单指令单数据流SISD：单一处理器执行单一指令流操作保存于单一存储上的数据，如单处理器系统
      2. 单指令多数据流SIMD：一条机器指令控制几个处理不见基于锁步方式同时执行，每个处理部件有一个相关的数据存储器。如向量和阵列处理器
      3. 多指令单数据流MISD：一系列数据被发送到一组处理器，每个处理器执行不同的指令序列。商业上不曾实现过
      4. 多指令多数据流MIMD：一组处理器同时执行不同的指令序列，对不同的数据集进行操作。如SMP、集群系统和NUMA系统。
         1. 处理器是通用的，每个处理器都能处理所有完成相应数据变换的必要指令
         2. 能以处理器的通信方式来进一步划分。如果处理器共享一个公共的存储器，则每个处理器存取共享存储器的程序和数据，并经由此存储器相互通信。这种系统的的最普通形式是对称多处理器SMP，存储器任何区域的存取时间对各处理器大致是相同的，也存在一种新的实现方法称为非均匀存储器访问NUMA
         3. 一组独立的单处理器或SMP，可互联成集群系统。计算机之间通过固定的路径或某些网络设施通信
   2. 并行组织
2. 对称多处理器SMP
   1. 特征
      1. 有两个或更多功能相似的处理器
      2. 这些处理器共享同一主存和IO设备，以总线或其他结构互联
      3. 所有处理器共享对IO设备的访问，或通过同一通道，或通过提供到同一设备路径的不同通道
      4. 所有处理器能完成同样的功能
      5. 系统被一个集中式操作系统OS控制，OS提供各处理器及其程序之间的作业级、任务级、文件级和数据元素级的交互
   2. 相对于单处理器潜在的的优点，并不保证必有，操作系统必须提供相应工具和功能以开发SMP系统的并行性
      1. 性能
      2. 可用性
      3. 增量式增长
      4. 可扩展
   3. 组织
   4. 多处理操作系统设计考虑
      1. 同时并发进程：OS例程必须是可重入的，以允许几个处理器可同时执行同一OS代码。当多个处理器执行OS的同一部分或不同部分时，要恰当的管理OS的表和其他数据结构，以避免死锁或无效操作
      2. 调度：任何处理器都可以完成调度，因此必须避免冲突。调度程序必须将就绪进程指派给可用处理器
      3. 同步：由于多个活动进程可能访问共享地址空间，或IO资源，因此必须提供有效的同步
      4. 存储管理：多处理器上的存储管理必须解决单机系统上的所有问题，还要充分发掘硬件并行性、协调不同处理器上的分页机制
      5. 可靠性和容错：面对处理器故障，操作系统应提供优雅的降级使用，调度程序和操作系统的其他部分必须识别处理器的失效，并相应的重构管理表
   5. 大型机SMP
3. cache一致性和MESI协议
   1. 问题：同一数据的多个副本可能同时存在于不同的cache中。若允许处理器自由的修改它们自己的副本，就会导致不同cache对存储器中同一数据的反应不一致
   2. 软件解决方法：一是禁止共享数据被缓存，而是分析共享变量的安全期并在编译程序生成代码时插入指令维护其在临界区的一致性
   3. 硬件解决方法：对运行时潜在的不一致情况动态识别，具体细节与实现方案有关。通常分为2类：
      1. 目录协议
         1. 主存控制器附加有一集中式控制器，保存各个局部cache内容的全局状态信息，也负责状态信息的更新
         2. 当某个cache控制器产生一个访问请求时，集中控制器检查此请求并发出必要的命令，以在存储器和cache、cache和cache之间传送数据
         3. 当处理器向局部cache行部分写入之前，必须向控制器请求排他性存储权。集中处理器授权前，会通知所有包含该行副本高速缓存的处理器该副本失效，得到所有处理器返回的认可后，控制器才将排他性存取权授予给请求的处理器
         4. 当一行已授权给某处理器专有，而另外的处理器企图读此行时，它将发送一个未命中提示给控制器。控制器通知授权处理器，要求将此行内容写入主存
         5. 此方法存在中央瓶颈，通信开销较大。但在使用了多条总线或某种另外的复杂互联结构的大型系统中，是很有效的
      2. 监听协议
         1. 将维护cache一致性的责任分布到多处理器的各个cache控制器上，一个cache必须识别出合适它保有的一行是与其他cache共享的。当对共享cache更新时，它通过一种广播机制通知其他cache
         2. 监听协议适用于基于总线的多处理器，但是必须避免因为广播和监听的总线传输抵消掉局部cache的收益
         3. 监听协议有2种基本方法
            1. 写-作废：某个cache要对此行执行更新时，先发送一个通知，以使其他cache中此行失效，使此行称为要执行写操作的cache专有。一旦变为专有，游泳者处理器可以进行本地写，直到其他处理器要求同一数据行
            2. 写-更新：当一处理器打算修改一个共享行时，被修改的特定数据字被广播给其他cache，于是包含此行的cache能同时进行写入修改
   4. MESI协议（写-作废）
      1. 数据cache每行维护一个两个状态位的标记
         1. 修改态Modified：此cache已被修改（不同于主存），并仅在这个cache中
         2. 专有态Exclusive：此cache行同于主存，但不出现于其他cache中
         3. 共享态Shared：此cache行同于主存，并可出现于另外的cache中
         4. 无效态Invalid：此cache行不含有效数据
      2. 状态转换
         1. 读缺失：一个cache未命中时，处理器启动一个存储器读，以读取包含有此未命中地址的主存行。处理器在总线上发出一个信号，通知其他处理器/cache单元监听此事务。这是会有几种可能
            1. 另一处理器持有此行的未修改，并处于专有态，则返回一个信号，指示它共享此行。2个cache的状态都置为共享
            2. 一个或多个cache持有此行的干净副本，则所有cache的状态都置为共享
            3. 另一处理器持有此行修改过的副本，则此cache阻塞存储器读，并将此行通过总线提供给发出请求的cache。存储器将数据接收并处理，2个cache的状态将置为共享
            4. 若无其他cache持有此行的副本，则没有信号返回。启动处理器读入此行，并将此行状态由无效转为专有
         2. 读命中：状态无改变，仍保留修改、共享、专有状态
         3. 写缺失：局部cache出现写缺失时，处理器启动一个存储器读，来读取含有此未命中地址的主存行。处理器发出一个"用于修改的读"信号。当此行装入cache后，将标记为修改态。数据装入时有2种可能
            1. 另一cache有此行的修改副本（状态为修改态），发起请求的cache放弃总线并等待，直到修改态的cache将副本写回主存，并将状态置为无效。然后再发起一次通知信号，从主存读取改行，并标记为修改态
            2. 其他cache没有修改过副本，cache读入行并进行修改。在此期间，其他处于共享态的cache将标记为无效
         4. 写命中：当写命中出现在当前cache时，效应取决于局部cache中此行的当前状态：
            1. 当前cache行状态为共享，则cache通过总线广播独占信号，其他含有此行副本的cache将各自的共享变成无效。然后cache完成修改，并将它的cache行状态改成M
            2. 当前cache行状态为E，则直接完成修改，并将状态改成M
            3. 当前cache行状态为M，只需完成修改
         5. L1-L2 cache一致性（上面提到的cache是连接到总线的cache，而L1通常不直接连接总线）
            1. 每个L1cache行都包括指示状态的位，如果一行同时出现在L1、L2，那么L1行的状态应追踪L2的状态。可采用写直达，或回写策略
4. 多线程和卡片上多处理器
   1. 隐式和显式多线程
      1. 概念
         1. 进程：在计算机上运行的一个程序实例，它体现2个主要特征
            1. 资源占有：进程包括一个包含进程映像的虚拟地址空间；进程映像是定义进程的程序、数据、栈和属性的集合。在不同的时刻，进程分配到的主存、IO设备、文件控制权也会变化
            2. 调度/执行：一个进程的执行沿一条执行路径流经一个或多个程序。这个执行可能与其他进程交错。于是，一个进程具有各种执行状态和派发优先权。晋城市操作系统调度和派发的一个实体
         2. 进程切换：将处理器由一个进程转换到另一个进程的操作。它通过保存第一个进程的所有控制数据、寄存器和其他信息，并以第二个进程的信息来替换他们而完成
         3. 线程：进程内可派发的任务单位。它包括一个处理器上下文（程序计数器和栈指针等）和它专有的栈数据区域。线程顺序执行并可中断
         4. 线程切换：在同一个进程中，将处理器控制由一线程转换到另一线程的动作。代价比进程切换小的多，因为同一进程中多线程共享资源
         5. 通常来说，线程关注调度和执行，进程关注调度/执行和资源占有。应用区分用户级线程和内核级线程，两者都是显式线程（因为由软件定义）
         6. 隐式多线程化指由单个顺序程序中提取多个线程来并发执行，这些隐式线程可静态的被编译器定义或动态的被硬件定义
   2. 显式多线程的方式
      1. 4种方法
         1. 交互式多线程，又称为细粒度多线程。处理器同时两个或多个线程上下，每个时钟周期由一个线程切换到另一个线程。若由于数据相关性或存储器等待而使一个线程阻塞，则跳过此线程去执行另一个就绪线程
         2. 阻塞式多线程，又称为粗粒度多线程。线程的指令连续的执行，知道如cache缺失这类引起延迟的事件出现，这类事件引发处理器切换到另一线程。这类方法在按序执行的处理器上比较有效
         3. 同时多线程SMT：来自多线程的指令同时发射到超标量处理器的执行单元，它将超标量多条指令发射的能力与多个线程上下文的使用相结合
         4. 片上多处理：是完整处理器在单一芯片上进行复制，每个处理器处理一个分立线程。这种方法的优点是，能有效的使用芯片上可用逻辑面积而不依赖复杂性日益增长的流水线设计
      2. 多线程化可能的流水体系结构
         1. ![多线程化可能的流水体系结构](../img/comp/concurrent-multipute-thread-data-flow.png)
   3. 示例系统
      1. Pentium 4：使用一种在Intel文档中称为超线程的多线程技术，实际上使用了支持2个线程的SMT，这样，单个多线程处理器逻辑上是2个处理器
      2. IBM Power 5：结合SMT个片上多处理器。芯片上具有两个分立的处理器，每个处理器使用SMT并发的支持两个线程的多线程处理器。设计者模拟证明，发现单个芯片上两个两路的SMT处理器比单个四路SMT处理器性能更优异；支持超出两个线程的额外多线程化因为线程切换导致cache频繁切换会降低性能
5. 集群
   1. 优点或设计目标
      1. 绝对的可扩展性
      2. 增量的可扩展性
      ...
      4. 游戏的价格/性价比
   2. 集群配置
      1. 被动式备用：一台计算器处理工作，一台不工作，通过心跳连接。一旦心跳停止，备用服务器任务主服务器失败，并立即投入运行。两者不共享磁盘，通常不被看作集群系统
      2. 主动式辅助
         1. 分立的服务器：每台计算器带有独立磁盘，且互相同步数据。调度软件将请求指派给某台计算机，并在故障时切换计算机
         2. 无共享：集群公共磁盘划分成卷，每卷被单一计算机拥有。故障时重新配置使另台计算机拥有失效计算机的卷
         3. 共享存储：每台计算机可存取共享磁盘所有卷，通过锁机制保证数据同步
      3. ![集群方法优缺点](../img/comp/concurrent-cluster-config-cpmaring.png)
   3. 操作系统设计问题
      1. 故障管理：高可用的集群系统和容错系统
      2. 负载均衡
      3. 并行化计算
         1. 并行化编译器：编译时确定应用哪些部分可并行执行，然后指派给不同的计算机。性能取决于问题的性质和编译器设计是否恰当
         2. 并行时应用：程序员从一开始就按照程序在寄存上运行的设想来编写程序，并使用消息传递方式在各结点网络间传送所要求的数据
         3. 参数式计算：如果应用的实质是一个算法或程序的多次执行，每次有一组不同的起始条件或参数，可以使用这种方法
   4. 集群计算机体系结构
      1. 单一入口点：用户登录到集群上而不是到个别计算机上
      2. 单一文件层次：用户看到的是，同一根目录下的单一文件目录层次体系
      3. 单一控制点：默认的工作站用于整个集群的控制和管理
      4. 单一虚拟网络：集群中单一结点能访问集群中任何其他节点
      5. 单一存储空间：分布式共享存储器允许程序共享变量
      6. 单一作业管理系统：在集群作业调度程序管理下，用于提交作业无需指定执行作业的宿主机
      7. 单一用户接口：一个公共图形接口支持所有用户，不管用户由那台工作站进入集群
      8. 单一IO空间：任意结点都可以远程访问任何IO外围或磁盘设备，而无需知道它们的物理位置
      9. 单一进程空间：使用一致的进程标识方式，任何结点上进程都在远程结点上生成另一进程并与之通信
      10. 检查点：这个功能周期的保存进程状态和中间计算结果，以便在故障修复后重新运算
      11. 进程迁移：实现负载均衡
      12. ![集群方法优缺点](../img/comp/concurrent-cluster-structure.png)
   5. 刀片服务器：是一种服务器体系结构，将多个服务器模块装入到单个机箱中。这种结构广泛用于数据中心，以节省空间、提高系统管理效率
   6. 和SMP对比
      1. SMP更容易配置和管理；更接近单处理器模型，几乎所有的应用以此模型编写；占用较少的物理空间，耗电较少；SMP产品已经建立且稳定
      2. 集群的增量和绝对可扩展性更好，可用性优
6. 非均匀存储器访问
   1. 概念
      1. 均匀存储器访问UMA：所有处理器都可使用转载和保存指令存取主存的所有部分，任一处理器对所有存储器的访问时间是相同的
      2. 非均匀存储器访问NUMA：所有处理器使用装载和保存指令能访问所有主存部分，根据正被访问的存储器区域，一个处理器所用的存储器访问时间是不同的。哪个区域快，哪个区域慢，对不同处理器是不同的
      3. cache一致的CC-NUMA：在各处理器的cache之间有维护cache一致性机制的NUMA系统（商业使用较多）
   2. 动机：SMP系统可使用的处理器数目是有限制的，有效的cache策略能减少任一处理器与主存之间的总线流通量。但随着处理器增多，总线逐渐称为瓶颈，SMP可配置的处理器大致在16-64之间。NUMA的目标是维护一个透明的、系统范围的存储器，并准许有许多个处理器结点，每个结点有自己的总线或其他内部互联系统
   3. CC-NUMA组织
      1. CC-NUMA有多个独立的结点，每个结点实际上是一个SMP组织，每个结点有多个处理器和主存，每个处理器有自己的L1和L2 cache。对于处理器来说，整个系统只有每一位置都具有唯一系统范围单一可寻址的存储器
      2. 结点通过某种通信设施互联，这些设施可以是开关结构、环或其他某种网络设施
   4. NUMA优缺点
      1. 优点：性能比SMP更优，不要求软件进行大的修改；
      2. 缺点：不如SMP透明；第二点与可用性有关，取决于CC-NUMA实现；如果存储器访问都是远程访问，性能会变差，可通过缓存设计或虚拟存储器进行优化
7. 向量计算
   1. 向量计算的方法
      1. 向量处理
      2. 并行处理
...
### 多核计算机
1. 概念：多核计算机也称为单芯片多处理器，指在一个单独的硅片上结合两个或多个处理器（称为核）。每个核由一个独立处理器的所有组件组成，如寄存器组、ALU、流水线硬件以及控制单元
2. 硬件性能问题
   1. 增加并行
      1. 流水线
      2. 超标量
